...
var DND = (function () {
   ...
   * @return {Character}
   */
  DND.getCharacterByName = function getCharacter(name) {
    var results = [];
    for (var i = 0; i < playerCharacters.length; i++) {
      if (playerCharacters[i].charName.toLowerCase() == name.toLowerCase()) {
  ...
  DND.getCharacterByID = function getCharacterByID(id) {
    for (var i = 0; i < playerCharacters.length; i++) {
      if (playerCharacters[i].charID == id) {
  ...
  class Character {
    constructor(charName, charClass, xp, level) {
      ...
      // Character ID for the database
      // If the character nr is less than 10, add a 0
      this.charID = DND.getCharacters().length < 10 ?
      ('0' + (DND.getCharacters().length+1).toString()+this.charName.substring(0,2)).toLowerCase() :
      ((DND.getCharacters().length+1).toString()+this.charName.substring(0,2)).toLowerCase();
    ...
    setImage(original) {
      this.image = name;
      writeImageToDisk(original, this.charID);
    }
    ...
    getImage() {
      return electron.remote.app.getPath('userData') + '/images/' + this.charID;
    }
  }
  ...
  DND.dbCharacters = function dbCharacters() {
    var results = [];
    var db = config.get('playerCharacters');
    //console.log(db);
    for (var i = 0; i < db.length; i++) {
      var c = db[i];
      results.push(this.newCharacter(c.charName, c.charClass, c.xp, c.level, c.image));
    }
    return results;
  }
...
function writeImageToDisk(targetImage, imageName) {
  var imageName = electron.remote.app.getPath('userData') + '/images/' + imageName;
  var image = fs.readFile(targetImage, function(err, data) {
